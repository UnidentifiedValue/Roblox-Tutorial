...
В прошлой версии структура была сильно смешана, поэтому тут я буду стараться
...
А ну и btw часть того что я здесь написал является краткой сводкой/дополненем
...
- **number** - число
  ...
  > в пределы приравнивается значение inf или -inf
...
- **tables** - структуры данных, вида `{[index] = value}`. Есть два подвида
  tables:
  - **arrays** - индексами являются ТОЛЬКО цифры, все элементы идут непрерывно и
    последовательно, для управления используется библиотека `table`.
...
- ### Структуры управления:
  ...
  > бесконечный цикл не имеет в себе задержки по времени, то компилятор
  > стремится выполнить код сразу, что ведет к остановке обработки всего кода
  > ради этого цикла, который никогда не завершится. В случае допущение подобной
  > ошибки и запуска студии, она сначала пролагает с минуту, а потом крашнется /
  > экстренно остановит выполнение кода
...
- ### **Функции
  ...
  Функции также могут возвращать несколько результатов одновременно, например:
  ```lua
    local function giveNextNumbers(n)
      return n, n+1, n+2
    end

    -- В данном случае можно за раз присвоить от 1 до 3х значений функции
    local num = giveNextNumbers(10) -- num = 10
    local x, y, z = giveNextNumbers(5) -- x = 5, y = 6, z = 7
    local a, b = giveNextNumbers(3) -- a = 3, b = 4
  ```
...
- ### **Задание типов:
  ...
  - Создания переменных с этим типом
    ```lua
    local vec : Vector2 = {
      x = 1,
      y = 2
  ...
  Если переменная не обязательно существует, то после её типа добавляется `?`, а
  если тип переменной может быть любым, то используется тип `any`.
  ...
  -- В данном случае при создании переменной типа Holder не обязательно добавлять поле content.

  local h1 : Holder = {id = 1},
  local h2 : Holder = {id = 2, content = 3}
  local h3 : Holder = {id = 3, content = "Hello"}
  ```
...
[это](https://create.roblox.com/docs/en-us/projects/data-model) и
[это](https://create.roblox.com/docs/en-us/projects/client-server), тк даже
несмотря на большое количество текста, это может дать очень хорошее
представление о том что вообще происходит.

В своей версии основ, я попытаюсь кратко (и надеюсь понятно) рассказать об
устройсте Roblox Studio, однако в случае чего - смотрите в интернете ( или
спрашивайте у меня)) ) вещи, которые все еще остаются непонятными.

> ЭТА ВЕРСИЯ ТУТОРИАЛА ПРЕДПОЛАГАЕТ ИСПОЛЬЗОВАНИЕ СТАРОГО ИНТЕРФЕЙСА СТУДИИ (НЕ
> NEW-GEN STUDIO PREVIEW) ПО ПРИЧИНЕ НЕПЕРЕНОСИМОСТИ АВТОРА КАЧЕСТВА НОВОГО
> ИНТЕРФЕЙСА

### Интерфейс:

Первое что бразается в глаза при открытии Roblox Studio - это большое количество
непонятных кнопок. Давайте разбираться.

- Viewport - центральная область, где показан игровой мир.
- Explorer - представляет все существующих обьекты в игре в виде древа.
- Output - окно вывода (его также можно открыть в игре на F9), именно в нем
  пишутся ошибки, сообщения и тд.
- Properties - свойства выделенного обьекта (выделение происходит нажатием M1 на
  обьект либо в Viewport, либо в Explorer)
- Top panel - набор инструментов для взаимодействия с Roblox Studio
  > Именно через неё открывается все что угодно

Сейчас мы подробно рассмотрим эти окна.

> А еще вы можете почитать более полную документацию
> [тут](https://create.roblox.com/docs/studio/ui-overview)

### Viewport:

В этом окне виден игровой мир так, как видит его игрок. Управление происходит
стандартно на WASD, камера вращается на зажатие ПКМ, обьекты выделяются ЛКМ. При
нажатии на обьект ПКМ всплывают опции, что с этим обьектом делать.

### Explorer:

Все существующие обьекты отображаются именно в окне Explorer. Exporer содержит в
себе четкую иерархию, от сервисов и до отдельных Instance.

Instance - это базовый класс любого обьекта в Roblox Studio, а сервис содержит в
себе API для работы с какой то частью функционала.

> Например `Players` сервис хранит в себе методы взаимодействия с игроками, а
> `Workspace` - методы взаимодействия с обьектами в мире.

Для создания нового обьекта в каком либо их сервисов, нажмите на `+`,
всплывающий при его выделении. Дальше выберете желаемый обьект, и он появится
как child этого сервиса. Эти обьекты можно перетаскивать, удалять и тд. Также,
рекомендую группировать Instances одного предназначения в папки (`Folder`),
чтобы потом было легче разбираться.

### Output:

Просто консольный вывод сообщений. В фильтрах можно отсортировать какие
сообщения показываются по типу сообщения (ошибка, информация, логи,
предупреждения и тд) и по тому откуда это сообщение было отправленно (с сервера,
клиента, самой студии и тд).

### Properties:

Это окно содержит все свойства выделенного обьекта. Для изменения какого либо
свойства нажмите на поле рядом с ним и введите / выберите новое значение. Всех
свойств ОЧЕНЬ много, поэтому читайте отдельно про свойства класса, который вам
нужен в процессе написания кода.

Также, в нем хранятся тэги и аттрибуты обьекта, это важно, но об этом подробнее
позже.

### Top panel:

На этой панели хранятся все самые важные кнопки. Она разделена на несколько
разделов

- HOME - универсальные инструменты, часто используемые вне зависимости от
  области деятельности
- MODEL - инструменты, полезные при постройке, редактировании в viewport
- AVATAR - инструменты создания, анимирования персонажа
- TEST - инструменты тестирования (например позволяют увидеть как игра будет
  выглядеть на разных устройствах)
- VIEW - самая заполненная панель, нужна для дебага, открытия всех других окон
  (типо output, exporer, toolbox)
- PLUGINS - установленные плагины

Отдельно напишу по скрытые панели, всплывающие только при необходимости

- UI - создание обьектов для интерфейса
  > Показывается при выборе StarterGui
- SCRIPT - инструменты для работы со скриптами
  > Показываются при открытии скрипта
